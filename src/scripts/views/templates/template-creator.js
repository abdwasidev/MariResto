...
const createRestaurantsDetail = (getRestoData) => `
    <section>
        ...
        <img class="lazyload" data-src="${getRestoData.pictureId ? CONFIG.BASE_IMAGE_URL + getRestoData.pictureId : 'https://picsum.photos/id/666/800/450?grayscale'}" alt="${getRestoData.name}/>
...
const createRestaurantsItemCard = (getRestoData) => `
    <div class="resto-card">
        <img class="lazyload" data-src="${getRestoData.pictureId ? CONFIG.BASE_IMAGE_URL + getRestoData.pictureId : 'https://picsum.photos/id/666/800/450?grayscale'}" alt="${getRestoData.name}"/>
        <a href="${`/#/detail/${getRestoData.id}`}"><h3 class="resto-name"><p>${getRestoData.name || '-'}</p></h3></a>
        <article class="detail">
            <div><i class="fa fa-star rating"></i><b> ${getRestoData.rating || '-'}</b></div>
            <div><i class="fa fa-location-dot city"></i> <b>${getRestoData.city || '-'}</b></div>
        </article>
        <p class="limit-teks">${getRestoData.description || '-'}</p>
    </div>
`;
